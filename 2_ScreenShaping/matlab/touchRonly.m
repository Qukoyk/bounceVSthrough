...
function varargout = touchRonly(varargin)
% TOUCHRONLY MATLAB code for touchRonly.fig
%      TOUCHRONLY, by itself, creates a new TOUCHRONLY or raises the existing
%      singleton*.
%
%      H = TOUCHRONLY returns the handle to a new TOUCHRONLY or the handle to
%      the existing singleton*.
%
%      TOUCHRONLY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TOUCHRONLY.M with the given input arguments.
%
%      TOUCHRONLY('Property','Value',...) creates a new TOUCHRONLY or raises the
...
% Edit the above text to modify the response to help touchRonly
...
% --- Executes just before touchRonly is made visible.
function touchLonly_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to touchRonly (see VARARGIN)

% Choose default command line output for touchRonly
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes touchRonly wait for user response (see UIRESUME)
...
% データ保存
dataMatrix = [leverPressData',leverData',timeData',leverActData'];
filter = {'*.csv'};
