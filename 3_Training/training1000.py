...
def through(ballLeftWidth,ballRightWidth):
    ballLeftPosition = int(256)
    ballRightPosition = int(768)
    height = int(360)
    screen.fill(BLACK)
    ballLeft = pygame.draw.circle(screen, WHITE, [ballLeftPosition,height], 30, ballLeftWidth)
    ballRight = pygame.draw.circle(screen, WHITE, [ballRightPosition,height], 30, ballRightWidth)
...
def bounce(ballLeftWidth,ballRightWidth):
    stage = ""
...
while True:
    listen()
    if randList[randPosition] == 1 and randList[randPosition+1] == 1 and randPosition < 999:
...
try:
    while timeNow - timeStart < timeMax:
        ...
        # gogogoリセット（視覚刺激後ボタン版のみ）    
        gogogo = 0

        # 乱数列から読み取って，0/1によって視覚刺激を呈示
        if btList[listPosition] == 0:
            if randList[listPosition] == 0:
                through(circleWidth,ringWidth)
                pass
            else:
                through(ringWidth,circleWidth)
                pass
            pass
        else:
            if randList[listPosition] == 0:
                bounce(circleWidth,ringWidth)
                pass
            else:
                bounce(ringWidth,circleWidth)
                pass
            pass
